...
onMounted(()=>{
    ...
    administrador.value = usuarioPinia.usuario.rol == "Administrador";
    moderador.value = usuarioPinia.usuario.rol == "Moderador";
    vetado.value = usuarioPinia.usuario.rol == "Vetado";
    getAllChildrens();
    getAllMonitores();
    getNotAssignedChild();
    getAssignedChildren();
...
<template>
    <div v-if="passByScreen" v-show="recarga">
        <h1 class="cabecera-texto">Bienvenido/a {{ usuarioPinia.usuario.nombre }} </h1>
